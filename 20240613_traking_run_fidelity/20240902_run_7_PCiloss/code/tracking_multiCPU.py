...
def select_model(name_model):
    ...
    il_mmi = 10**(il_mmi_dB/10)
    imbalance = 10**(imbalance_dB/10)
    il_cross = 10**(il_cross_dB/10)
    ...
    mmi_i_losses_mtx_even = torch.full((2*(n_inputs-1), n_inputs//2), il_mmi)
    mmi_i_losses_mtx_odd = torch.full((n_inputs, n_inputs//2-1), il_mmi)
    ...
    crossing_i_losses_mtx_odd = torch.full((n_inputs-2, n_inputs//2-1), il_cross)
    crossing_crosstalks_mtx_odd = torch.full((n_inputs-2, n_inputs//2-1), cross_talk)
    crossing_i_losses_mtx_odd_side = torch.full((n_inputs-2, n_inputs//2+1), il_cross)
    ...
    if name_model == 'Clements_Arct':
        ...
        model = NEUROPULSBellCrossingSide_Arct(
            ...
            crossing_i_losses_mtx_odd=crossing_i_losses_mtx_odd_side,
            crossing_crosstalks_mtx_odd=crossing_crosstalks_mtx_odd_side)
    elif name_model == 'NEUROPULSBellCrossingSide2_Arct':
...
if __name__ == "__main__":
    ...
    parser.add_argument('--il_mmi', required=True, help='il_mmi in dB')
    parser.add_argument('--imbalance', required=True, help='imbalance in dB')
    parser.add_argument('--il_cross', required=True, help='il_cross in dB')
    ...
    il_mmi_dB = float(args.il_mmi)         # =P_out/P_in. 0dB pefect component, -100dB very lossy
    imbalance_dB = float(args.imbalance)   # =P_outmax/P_outmin. 0dB 50/50 MMI, 100dB all power to outUP, -100dB all power to outDOWN
    il_cross_dB = float(args.il_cross)         # =P_out/P_in. 0dB pefect component, -100dB very lossy
    ...
    name_folder_out = 'n'+str(n_inputs)+"_pciloss"+str(pc_iloss_dB)+'_ilmmi'+str(il_mmi_dB)+'_imb'+str(imbalance_dB)+'_ilcross'+str(il_cross_dB)+'_crosstalk'+str(cross_talk_dB)+'_HPC_simulation/'
    # =============================================================================================================
    # =============================================================================================================
    # =============================================================================================================
