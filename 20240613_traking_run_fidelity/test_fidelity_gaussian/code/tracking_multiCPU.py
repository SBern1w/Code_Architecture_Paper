...
def select_model(name_model):
    gaussian_dB = create_truncated_gaussian_tensor(pc_iloss_mu, pc_iloss_sigma, (2*(n_inputs-1), n_inputs), 0)
    pc_i_losses_mtx_even = 10**(gaussian_dB/10)
    gaussian_dB = create_truncated_gaussian_tensor(pc_iloss_mu, pc_iloss_sigma, (n_inputs, n_inputs), 0)
    pc_i_losses_mtx_odd = 10**(gaussian_dB/10)
    gaussian_dB = create_truncated_gaussian_tensor(pc_iloss_mu, pc_iloss_sigma, (2, n_inputs), 0)
    pc_i_losses_mtx_inout = 10**(gaussian_dB/10)
    gaussian_dB = create_truncated_gaussian_tensor(pc_iloss_mu, pc_iloss_sigma, (n_inputs, n_inputs), 0)
    pc_i_losses_mtx_full = 10**(gaussian_dB/10)
    gaussian_dB = create_truncated_gaussian_tensor(pc_iloss_mu, pc_iloss_sigma, (n_inputs//2, n_inputs), 0)
    pc_i_losses_mtx_side = 10**(gaussian_dB/10)
    gaussian_dB = create_truncated_gaussian_tensor(i_loss_MMI_mu, i_loss_MMI_sigma, (2*(n_inputs-1), n_inputs//2), 0)
    mmi_i_losses_mtx_even = 10**(gaussian_dB/10)
    gaussian_dB = create_truncated_gaussian_tensor(i_loss_MMI_mu, i_loss_MMI_sigma, (n_inputs, n_inputs//2-1), 0)
    mmi_i_losses_mtx_odd = 10**(gaussian_dB/10)
    gaussian_dB = create_truncated_gaussian_tensor(imbalance_mu, imbalance_sigma, (2*(n_inputs-1), n_inputs//2))
    mmi_imbalances_mtx_even = 10**(gaussian_dB/10)
    gaussian_dB = create_truncated_gaussian_tensor(imbalance_mu, imbalance_sigma, (n_inputs, n_inputs//2-1))
    mmi_imbalances_mtx_odd = 10**(gaussian_dB/10)
    gaussian_dB = create_truncated_gaussian_tensor(i_loss_Crossing_mu, i_loss_Crossing_sigma, (n_inputs-2, n_inputs//2-1), 0)
    crossing_i_losses_mtx_odd = 10**(gaussian_dB/10)
    gaussian_dB = create_truncated_gaussian_tensor(i_loss_Crossing_mu, i_loss_Crossing_sigma, (n_inputs-2, n_inputs//2+1), 0)
    crossing_i_losses_mtx_odd_side = 10**(gaussian_dB/10)
    gaussian_dB = create_truncated_gaussian_tensor(cross_talk_mu, cross_talk_sigma, (n_inputs-2, n_inputs//2-1))
    crossing_crosstalks_mtx_odd = 10**(gaussian_dB/10)
    gaussian_dB = create_truncated_gaussian_tensor(cross_talk_mu, cross_talk_sigma, (n_inputs-2, n_inputs//2+1))
    crossing_crosstalks_mtx_odd_side = 10**(gaussian_dB/10)
...
if __name__ == "__main__":
    ...
    i_loss_MMI_mu = -0.3        # Average =P_out/P_in. 0dB pefect component, -100dB very lossy
    i_loss_MMI_sigma = 0.2      # Std deviation
    imbalance_mu = 0            # Average =P_outmax/P_outmin. 0dB 50/50 MMI, 100dB all power to outUP, -100dB all power to outDOWN
    imbalance_sigma = 0.2      # Std deviation

    i_loss_Crossing_mu = -0.3       # Average =P_out/P_in. 0dB pefect component, -100dB very lossy
    i_loss_Crossing_sigma = 0.1     # Std deviation
    ...
    name_folder_out = 'n'+str(n_inputs)+'_pcilossmu'+str(pc_iloss_mu)+'_pcilosssigma'+str(pc_iloss_sigma)+'_HPC_simulation/'
    # =============================================================================================================
    # =============================================================================================================
    # =============================================================================================================
