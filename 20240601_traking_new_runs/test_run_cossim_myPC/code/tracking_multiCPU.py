...
# ABS of the Frobenius product of Cosine Similarity
# The loss should be negative so I put 1 - cosine 
class ABSCosineSimilarityFrobeniusLoss(nn.Module):
    def __init__(self):
        super(ABSCosineSimilarityFrobeniusLoss, self).__init__()
    ...
    def forward(self, predicted_matrix, target_matrix):
        ...
        Frobenius_module_t = torch.trace(torch.matmul(target_matrix.t().conj(), target_matrix))
        Frobenius_pt = torch.trace(torch.matmul(predicted_matrix.t().conj(), target_matrix))
        cosine_similarity = Frobenius_pt/torch.sqrt(Frobenius_module_p*Frobenius_module_t)
        module_cosine_similarity = torch.abs(cosine_similarity)   # Take the abs
        loss = 1 - module_cosine_similarity     # I shuould min the loss
        return loss
...
def select_model(name_model):
    ...
    if name_model == 'Clements_Arct':
        ...
        model = NEUROPULSBell_Arct(
            ...
            crossing_i_losses_mtx_odd=crossing_i_losses_mtx_odd,
            crossing_crosstalks_mtx_odd=crossing_crosstalks_mtx_odd)
    elif name_model == 'NEUROPULSBell_CrossingSide_Arct':
        model = NEUROPULSBell_CrossingSide_Arct(
            n_inputs=n_inputs,
            mmi_i_losses_mtx_even=mmi_i_losses_mtx_even,
            mmi_imbalances_mtx_even=mmi_imbalances_mtx_even,
            crossing_i_losses_mtx_odd=crossing_i_losses_mtx_odd_side,
...
def model_prediction(args):
    ...
    loss_fn = ABSCosineSimilarityFrobeniusLoss()
