...
class PCLayerMatrix_Full(nn.Module):
    ...
    def __init__(self,
                 n_inputs: int,
                 pc_loss_dB_max: float = 0.0,
                 pc_loss_dB_cost: float = 0.0,
                 N_bits: int = 8,
                 phase_shift_noise_std: float = 0.,):
        super(PCLayerMatrix_Full, self).__init__()
        if n_inputs%2 == 1: raise Exception('n_inputs is odd!!! NONONO, put it even!!!')
        self.alphaL = pc_loss_dB_max / 8.68589
        self.T0 = 10 ** (-pc_loss_dB_cost / 20)
        self.phase_shift = nn.Parameter(torch.pi*torch.rand(n_inputs, device=device), requires_grad=True)
    ...
    def forward(self, x_matrix):
        ...
        Transmission = self.T0 * torch.exp(-self.alphaL / (2*torch.pi) * phase_shift_quantized)
        layer = Transmission * torch.exp(1.j*phase_shift_quantized)
        return layer.view(-1, 1) * x_matrix
...
class PCLayerMatrix_Even(nn.Module):
    ...
    def forward(self, x_matrix):
        phase_shift_quantized = self.dac(self.phase_shift)
        Transmission = self.T0 * torch.exp(-self.alphaL / (2*torch.pi) * phase_shift_quantized)
        layer = torch.where(self.mask, Transmission*torch.exp(1.j*phase_shift_quantized), self.conn)
        return layer.view(-1, 1) * x_matrix
