...
class CrossingLayerMatrix_Odd(nn.Module):
    ...
    Where:
        0__   __0                              |  sqrt(CT)  | sqrt(1-CT) |
           \-/        =>   TF = sqrt(1-LOSS) x |            |            |
        1__/-\__1                              | sqrt(1-CT) |  sqrt(CT)  |
    """
    def __init__(self,
                 n_inputs: int,
        ...
        tau_tf = torch.sqrt(atten) * torch.sqrt(tau).to(torch.complex64)
        kappa_tf = torch.sqrt(atten) * torch.sqrt(kappa).to(torch.complex64)
...
class CrossingLayerMatrix_Odd_noTopBottom(nn.Module):
    r"""
    Create odd Crossings matrix
        0_______0
    ...
    def __init__(self,
                 ...
                 crossing_i_losses: torch.Tensor = None,
                 crossing_crosstalks: torch.Tensor = None):
        super(CrossingLayerMatrix_Odd_noTopBottom, self).__init__()
        if n_inputs%2 == 1: raise Exception('n_inputs is odd!!! NONONO, put it even!!!')
        _n_crossing = n_inputs//2 - 1
        if crossing_i_losses is None or crossing_crosstalks is None:
            atten = torch.ones(_n_crossing, device=device)
            ...
            atten = 1 - crossing_i_losses
            tau = crossing_crosstalks
            kappa = 1 - crossing_crosstalks
            edge_up_crossing = torch.Tensor([1.], device=device)
            edge_dw_crossing = edge_up_crossing
