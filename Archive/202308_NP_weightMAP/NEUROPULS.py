...
class _NEUROPULS_layer_wg_input(nn.Module):
    r""" Create NEUROPULS waveguide input
    0__[]__0
    ...
    def __init__(self, N: int = 3):
        super(_NEUROPULS_layer_wg_input, self).__init__()
...
class _NEUROPULS_layer_wg2(nn.Module):
    ...
    def __init__(self, N: int = 3):
        ...
        self.wg_delete = torch.ones(N, requires_grad=False, device=device)  # Delete the waveguide from parameters
        self.add_conns = torch.zeros(N, requires_grad=False, device=device) # Add the connections
        self.wg_delete[0] = 0.0
        self.add_conns[0] = 1.0
        self.wg_delete[-1] = 0.0
        self.add_conns[-1] = 1.0
...
class NEUROPULSNxN(nn.Module):
    r""" A approsimate unitary matrix network based on the NEUROPULS architecture.
    Network::

        0__[]__  __[]__________  __[]__________  ________4
               \/              \/              \/
        1______/\__[]__  __[]__/\__[]__  __[]__/\____  __5
                       \/              \/            \/
        2__[]__  __[]__/\__[]__  __[]__/\__[]__  ____/\__6
               \/              \/              \/
        3______/\______________/\______________/\________7
    ...
    """
    def __init__(self, N: int = 3):
        super(NEUROPULSNxN, self).__init__()
        self.N = N
        self.wg_input_layer = _NEUROPULS_layer_wg_input(N=N)
        self.wg1_layer = nn.ModuleList([_NEUROPULS_layer_wg1(N=N) for i in range((N+1)//2)])
        self.wg2_layer = nn.ModuleList([_NEUROPULS_layer_wg2(N=N) for i in range(N//2)])
        self.MMI1_layer = nn.ModuleList([_NEUROPULS_layer_MMI1(N=N) for i in range((N+1)//2+1)])
        self.MMI2_layer = nn.ModuleList([_NEUROPULS_layer_MMI2(N=N) for i in range(N//2+1)])

    def forward(self, x):
        x_complex = torch.complex(x, torch.zeros_like(x))
        # Circuit
        x_complex = self.wg_input_layer(x_complex)
        ...
        if self.N%2 == 0:
            x_complex = self.MMI1_layer[-1](x_complex)
            x_complex = self.MMI2_layer[-1](x_complex)
        else:
            x_complex = self.MMI1_layer[-1](x_complex)
            x_complex = self.wg1_layer[-1](x_complex)
            x_complex = self.MMI2_layer[-1](x_complex)
            x_complex = self.MMI1_layer[-1](x_complex)
        # DETECTOR
...
class NEUROPULSNxN_N(nn.Module):
    r""" A approsimate unitary matrix network based on the NEUROPULS architecture.
    Network::
    ...
    """
    def __init__(self, N: int = 3):
        super(NEUROPULSNxN_N, self).__init__()
        self.N = N
        self.wg_input_layer = _NEUROPULS_layer_wg_input(N=N)
        self.wg1_layer = nn.ModuleList([_NEUROPULS_layer_wg1(N=N) for i in range((N+1))])
        self.wg2_layer = nn.ModuleList([_NEUROPULS_layer_wg2(N=N) for i in range(N)])
        self.MMI1_layer = nn.ModuleList([_NEUROPULS_layer_MMI1(N=N) for i in range((N+1)+1)])
        self.MMI2_layer = nn.ModuleList([_NEUROPULS_layer_MMI2(N=N) for i in range(N+1)])
...
# N=6
# modulae = _NEUROPULS_layer_MMI2(N=N)
# input = torch.ones(N)
# modulae(torch.complex(input, torch.zeros_like(input)))
# # ---------------------------------------------------------------------------------------------------------------
