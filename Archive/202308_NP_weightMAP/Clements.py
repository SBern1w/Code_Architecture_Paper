# Create the model of Clements architecture
...
class _Clements_layer_wg1(nn.Module):
    r""" Create Clements waveguide layer 1
    0__[]__0
    ...
    def __init__(self, N: int = 3):
        super(_Clements_layer_wg1, self).__init__()
...
class _Clements_layer_wg2(nn.Module):
    r""" Create Clements waveguide layer 2
    0______0
    ...
    """
    def __init__(self, N: int = 3):
        super(_Clements_layer_wg2, self).__init__()
        self.phase_shift = nn.Parameter(2*torch.pi*torch.rand(N, device=device), requires_grad=True)
        # Waveguide connections
        self.wg_delete = torch.ones(N, requires_grad=False, device=device)  # Delete the waveguide from parameters
        self.add_conns = torch.zeros(N, requires_grad=False, device=device) # Add the connections
...
class _Clements_layer_MMI1(nn.Module):
    r""" Create Clements MMI layer 1
    0__  __0
       \/
    1__/\__1
    ...
    """
    def __init__(self, N: int = 3):
        super(_Clements_layer_MMI1, self).__init__()
...
class _Clements_layer_MMI2(nn.Module):
    r""" Create Clements MMI layer 2
    0______0
    ...
    """
    def __init__(self, N: int = 3):
        super(_Clements_layer_MMI2, self).__init__()
        MMI_device = np.sqrt(0.5)*torch.tensor([[1., 1.j],
                                                [1.j, 1.]],
                                                requires_grad=False)
        # MMI connections
...
class ClementsNxN(nn.Module):
    r""" A approsimate unitary matrix network based on the Clements architecture.
    Network::

        0__[]__  __[]__  __________________[]__  __[]__  __________________4
               \/      \/                      \/      \/
        1______/\______/\__[]__  __[]__  ______/\______/\__[]__  __[]__  __5
                               \/      \/                      \/      \/
        2__[]__  __[]__  ______/\______/\__[]__  __[]__  ______/\______/\__6
               \/      \/                      \/      \/
        3______/\______/\______________________/\______/\__________________7
    ...
    def __init__(self, N: int = 3):
        ...
        self.N = N
        self.wg1_layer = nn.ModuleList([_Clements_layer_wg1(N=N) for i in range(N)])   
        self.wg2_layer = nn.ModuleList([_Clements_layer_wg2(N=N) for i in range(N)])
        self.MMI1_layer = nn.ModuleList([_Clements_layer_MMI1(N=N) for i in range(N)])
        self.MMI2_layer = nn.ModuleList([_Clements_layer_MMI2(N=N) for i in range(N)])
