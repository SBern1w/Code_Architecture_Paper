...
if __name__ == "__main__":
    ...
    parallel_function_remote = ray.remote(model_prediction)
    ...
    for N in Ns:
        for i_matrix in tqdm.trange(n_matrices):
            ...
            # target_predition_loss = []
            # with multiprocessing.Pool(processes=num_processors) as pool:       # Run in all the available processors
            #     target_predition_loss.append(pool.map(model_prediction, args_list))

            futures = [parallel_function_remote.remote(arg) for arg in args_list]
            target_predition_loss = ray.get(futures)
            ...
            np.save(folder_path+'202404115_simulation', TPL_np)
